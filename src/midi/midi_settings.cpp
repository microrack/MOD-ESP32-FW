...
MidiSettings::MidiSettings(Display* display, MidiSettingsState* state)
    : ScreenInterface(display), state(state), current_item(MENU_CHANNEL), blink_counter(0), is_editing(false) {}
...
void MidiSettings::render_menu() {
    const char* items[MENU_COUNT] = {"Channel: %s", "Out A: %s", "Out B: %s", "Out C: %s", "Clock: %s"};
...
void MidiSettings::handle_menu_input(Event* event) {
    if (event->encoder != 0) {
        if (is_editing) {
            ...
            switch (current_item) {
                ...
                case MENU_OUT_A:
                    state->set_midi_out_type(
                        0, (MidiOutType)clampi(state->get_midi_out_type(0) + event->encoder,
                                               state->get_min_midi_out_type(), state->get_max_midi_out_type()));
                    break;
                case MENU_OUT_B:
                    state->set_midi_out_type(
                        1, (MidiOutType)clampi(state->get_midi_out_type(1) + event->encoder,
                                               state->get_min_midi_out_type(), state->get_max_midi_out_type()));
                    break;
                case MENU_OUT_C:
                    state->set_midi_out_type(
                        2, (MidiOutType)clampi(state->get_midi_out_type(2) + event->encoder,
                                               state->get_min_midi_out_type(), state->get_max_midi_out_type()));
                    break;
                case MENU_CLOCK:
            ...
            // Move selection
            current_item = (MenuItems)clampi(current_item + event->encoder, 0, MENU_COUNT - 1);
        }
    }
}
