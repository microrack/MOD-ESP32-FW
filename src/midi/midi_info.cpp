...
MidiInfo::MidiInfo(Display* display, MidiSettingsState* state) : ScreenInterface(display), state(state) {
...
void MidiInfo::render() {
    ...
    sprintf(buffer, "Ch: %s  Clk: %s", state->get_midi_channel_str(), state->get_midi_clk_type_str());
    display->println(buffer);
    display->println();
...
void MidiInfo::handle_input(Event* event) {
    ...
    if (event->encoder != 0) {
        state->set_bpm(clampi(state->get_bpm() + event->encoder, state->get_min_bpm(), state->get_max_bpm()));
