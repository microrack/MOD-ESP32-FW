...
uint16_t OscilloscopeRoot::scale_to_rate(size_t scale_index) {
    return (uint16_t)(1.0 / (time_scales[scale_index] * 0.001 /
                             TICK_SPACING));
}

OscilloscopeRoot::OscilloscopeRoot(Display* display)
    : ScreenInterface(display) {
    signal_config.channel_count = 2;
    signal_config.channels[0] =
        static_cast<adc_channel_t>(ADC1_GPIO36_CHANNEL);
    signal_config.channels[1] =
        static_cast<adc_channel_t>(ADC1_GPIO37_CHANNEL);
    ...
    signal_config.buffer_size =
        TICK_SPACING * (SCREEN_WIDTH / TICK_SPACING);
...
void OscilloscopeRoot::drawGraph() {
    ...
    if (millis() - last_trigger_wait > 1000 || sigscoper.is_ready() ||
    ...
    display->printf("%.0f %s/d ",
                    ...
                    time_scales[current_scale_index] >= 1.0 ? "ms"
                                                            : "us");
    ...
    case DisplayMode::SINGLE:
        ...
        for (int i = 1; i < SCREEN_WIDTH - 2; i++) {
            ...
            int y1 =
                map(signal_buffer[i], 400, 2400, SCREEN_HEIGHT, 10);
            int y2 =
                map(signal_buffer[i + 1], 400, 2400, SCREEN_HEIGHT, 10);

            if (y1 >= 0 && y1 < SCREEN_HEIGHT && y2 >= 0 &&
                y2 < SCREEN_HEIGHT) {
                // Draw line with 3-pixel width
                display->drawLine(i, y1, i + 1, y2, SSD1306_WHITE);
                display->drawLine(i, y1 + 1, i + 1, y2 + 1,
                                  SSD1306_WHITE);
                display->drawLine(i, y1 + 2, i + 2, y2 + 2,
                                  SSD1306_WHITE);
            }
        }
        break;
    ...
    case DisplayMode::JOINED:
        ...
        for (int i = 1; i < SCREEN_WIDTH - 2; i++) {
            ...
            int y1 =
                map(signal_buffer2[i], 400, 2400, SCREEN_HEIGHT, 10);
            int y2 = map(signal_buffer2[i + 1], 400, 2400,
                         SCREEN_HEIGHT, 10);
    ...
    case DisplayMode::SPLIT:
        ...
        for (int i = 1; i < SCREEN_WIDTH - 2; i++) {
            ...
            int y1 =
                map(signal_buffer[i], 400, 2400, SCREEN_HEIGHT / 2, 10);
            int y2 = map(signal_buffer[i + 1], 400, 2400,
                         SCREEN_HEIGHT / 2, 10);

            if (y1 >= 0 && y1 < SCREEN_HEIGHT / 2 && y2 >= 0 &&
                y2 < SCREEN_HEIGHT / 2) {
                // Draw line with 3-pixel width
                display->drawLine(i, y1, i + 1, y2, SSD1306_WHITE);
                display->drawLine(i, y1 + 1, i + 1, y2 + 1,
        ...
        for (int i = 1; i < SCREEN_WIDTH - 2; i++) {
            ...
            int y1 = map(signal_buffer2[i], 400, 2400, SCREEN_HEIGHT,
                         SCREEN_HEIGHT / 2);
            int y2 = map(signal_buffer2[i + 1], 400, 2400,
                         SCREEN_HEIGHT, SCREEN_HEIGHT / 2);

            if (y1 >= SCREEN_HEIGHT / 2 && y1 < SCREEN_HEIGHT &&
                y2 >= SCREEN_HEIGHT / 2 && y2 < SCREEN_HEIGHT) {
                // Draw line with 3-pixel width
                display->drawLine(i, y1, i + 1, y2, SSD1306_WHITE);
                display->drawLine(i, y1 + 1, i + 1, y2 + 1,
                                  SSD1306_WHITE);
    ...
    if (!is_rolling(current_scale_index)) {
        if (display_mode == DisplayMode::SPLIT) {
            ...
            const int midY2 =
                3 * SCREEN_HEIGHT / 4; // Middle of lower half

            // Draw ticks for upper half (first channel)
            for (int x = SCREEN_WIDTH - 2; x >= 0; x -= TICK_SPACING) {
                for (int y = midY1 - TICK_SIZE / 2;
                     y <= midY1 + TICK_SIZE / 2; y++) {
            ...
            for (int x = SCREEN_WIDTH - 2; x >= 0; x -= TICK_SPACING) {
                for (int y = midY2 - TICK_SIZE / 2;
                     y <= midY2 + TICK_SIZE / 2; y++) {
                    display->drawPixel(x, y, SSD1306_WHITE);
                }
            }
            ...
            for (int x = SCREEN_WIDTH - 2; x >= 0; x -= TICK_SPACING) {
                for (int y = midY - TICK_SIZE / 2;
                     y <= midY + TICK_SIZE / 2; y++) {
                    display->drawPixel(x, y, SSD1306_WHITE);
                }
            }
...
void OscilloscopeRoot::update(Event* event) {
    ...
    if (event->encoder != 0) {
        ...
        // Increase index (slower time scale) when turned
        // counter-clockwise
        else if (event->encoder < 0 &&
                 current_scale_index < TIME_SCALE_COUNT - 1) {
            current_scale_index++;
        }
