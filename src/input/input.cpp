...
Event Input::get_inputs() {
    ...
    if (button_a_reading != last_button_a_state
        ...
        bool button_a_state =
            (button_a_reading == LOW);  // LOW means pressed (pull-up)
    ...
    // Handle Encoder Switch with immediate events but debounce
    // inhibition
    if (button_sw_reading != last_button_sw_state
        && (current_time - last_button_sw_change_time)
               > DEBOUNCE_TIME) {
        // State changed and debounce period passed since last change
        last_button_sw_change_time = current_time;
        last_button_sw_state = button_sw_reading;

        bool button_sw_state =
            (button_sw_reading == LOW);  // LOW means pressed (pull-up)
...
void Event::print(const Event& event) {
    if (event.encoder != 0 || event.button_a != ButtonNone
        || event.button_sw != ButtonNone) {
        Serial.print("Event: ");
        if (event.encoder != 0) {
